...
%
% Required packages (all in standard TeX Live / MiKTeX):
...
\definecolor{pfnavy}{RGB}{30, 58, 95}       % #1E3A5F
\definecolor{pfblue}{RGB}{37, 99, 235}      % #2563EB
\definecolor{pflight}{RGB}{239, 246, 255}   % #EFF6FF
\definecolor{pfgray}{RGB}{100, 116, 139}    % #64748B
\definecolor{pfgreen}{RGB}{5, 150, 105}     % #059669
\definecolor{pfred}{RGB}{220, 38, 38}       % #DC2626
...
% ── Beamer settings ──────────────────────────────────────────────────────────
\setbeamertemplate{navigation symbols}{}   % remove nav bar clutter
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        ...
        \item A bad prompt vs.\ a good prompt on the \textbf{same model} can differ by \textcolor{pfblue}{\textbf{60--70\%}} in quality
        \item No standard way to \textbf{compare}, \textbf{measure}, or \textbf{teach} prompt engineering
      \end{itemize}
    \end{column}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Syntax errors caught at compile
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Logic errors only visible at runtime
...
\begin{frame}{Seven Prompting Strategies}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Self-Reflection   & Generate → critique → revise             & High-stakes accuracy \\
...
\begin{frame}{PromptForge — System Architecture}
  ...
  \begin{tikzpicture}[
    ...
    label/.style={font=\scriptsize, color=pfgray}
  ]
    % Nodes
    \node[box] (input)    at (0, 0)   {User Input\\(Task + Text)};
    \node[box] (engine)   at (4, 0)   {Prompt Strategy\\Engine (7 builders)};
    \node[box] (api)      at (8, 0)   {OpenAI GPT\\(same model × 7)};
    \node[box] (eval)     at (8,-1.8) {Evaluation\\Engine};
    \node[box] (dash)     at (4,-1.8) {Streamlit\\Dashboard};

    % Arrows
    \draw[arrow] (input)  -- node[label, above]{task + text} (engine);
    \draw[arrow] (engine) -- node[label, above]{7 prompts}   (api);
    \draw[arrow] (api)    -- node[label, right]{7 responses} (eval);
    \draw[arrow] (eval)   -- node[label, below]{metrics}     (dash);
    \draw[arrow] (dash)   -- node[label, below]{charts}      (input);

    % Annotation
    \node[label, align=center] at (4, -2.9)
...
promptforge/
  app.py
  prompts/
    strategies.py  # 7 builders
    templates.py   # task configs
  engine/
    llm_client.py  # API wrapper
    evaluator.py   # quality metrics
  data/
    examples.py    # few-shot data
  .streamlit/
...
\begin{frame}{Evaluation Methodology — Without Using Another LLM}
  ...
  \begin{tabular}{@{}llrl@{}}
    ...
    JSON Validity            & Parseable \texttt{\{\}} block in response &  5 pts & Machine readability \\
    \midrule
...
\begin{frame}{Live Demonstration}
  ...
  \vspace{0.5em}
  \begin{center}
    \texttt{streamlit run promptforge/app.py} $\rightarrow$ \texttt{localhost:8501}
  \end{center}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{exampleblock}{Key Finding}
        ...
        \medskip
        Chain-of-Thought offers the best \textbf{quality-to-cost} tradeoff: +69\%
        improvement from adding 12 words to the prompt.
      \end{exampleblock}
      \vspace{0.5em}
...
def login(username, password):
    ...
    ...
      \end{lstlisting}
      \textcolor{pfred}{\small$\rightarrow$ Misses SQL injection entirely}
    \end{column}
    \begin{column}{0.48\textwidth}
...
{"severity": "...",
 ...
 "fix": "..."}
      \end{lstlisting}
      \textcolor{pfgreen}{\small$\rightarrow$ Flags CRITICAL SQL injection + fix}
    \end{column}
  \end{columns}
  \vspace{0.5em}
  \begin{center}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Agentic chaining (Zero-Shot → Self-Reflection → Structured)
...
\begin{frame}{References}
  ...
  \begin{itemize}
    ...
    \item Shinn, N.\ et al.\ (2023). \textit{Reflexion: Language Agents with Verbal Reinforcement Learning}. NeurIPS.
    \item Reynolds, L.\ \& McDonell, K.\ (2021). \textit{Prompt Programming for Large Language Models}. CHI EA.
    \item White, J.\ et al.\ (2023). \textit{A Prompt Pattern Catalog to Enhance Prompt Engineering}. arXiv.
    \item OpenAI (2024). \textit{GPT-4o mini model card}. platform.openai.com
    \item Flesch, R.\ (1948). A new readability yardstick. \textit{Journal of Applied Psychology}, 32(3).
  \end{itemize}
\end{frame}
...
\begin{frame}{Thank You}
  \begin{center}
    ...
    \begin{tabular}{rl}
      ...
      \textbf{Source code:} & \texttt{promptforge/} \\
